...
VOCAB_PATH = train_file + "_vocab.pkl"

with open(VOCAB_PATH, "wb") as f:
    pickle.dump(vocab_index, f)

print(f"[OK] Vocabulary saved to {VOCAB_PATH}")
...
with torch.no_grad():
    for feature, target in testloop:
        ...
        if model.name == 'cnn':
            # CNN outputs logits for 2 classes
            preds = torch.argmax(out, dim=1)
        else:
            # LSTM outputs probabilities
